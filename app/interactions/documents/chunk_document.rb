  ...
  class ChunkDocument < ApplicationInteraction
    ...
    def execute
      ...
      if document.content.blank?
        document.mark_failed!("Document has no content")
        errors.add(:base, "Document has no content")
        return
      end
    ...
    def create_chunks
      ...
      while position < content.length
        ...
        # Ensure we always advance position to avoid infinite loops
        next_position = chunk_end - chunk_overlap
        position = [next_position, position + 1].max
      end
